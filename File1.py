...
class MyApp(ttk.Frame):
    ...
    def app_save_omega(self):
        if 'omega_data' in self.__dict__:
            ...
            signal_calculator = SignalCalculator(self.k1, self.k2, self.k3, self.delta_t, self.start_time, self.end_time, self.xi, self.eta, self.right_frame)
    ...
    def app_check_input_data(self):
        ...
        xi_str = self.input_xi.get()
        eta_str = self.input_eta.get()
        try:
            ...
            self.xi = float(xi_str)
            self.eta = float(eta_str)
...
class SignalCalculator:
    def __init__(self, k1, k2, k3, delta_t, start_time, end_time, xi, eta, right_frame):
        self.k1 = k1
        self.k2 = k2
        self.k3 = k3
    ...
    def omega_1(self, t):
        term1 = self.eta * (2*self.k1 + self.xi*self.k2 + self.xi*self.k3) * np.cos((self.k2 - self.k3)*t)
        term2 = 0.5 * self.eta * (self.k2 + self.k3) * (1 - self.xi) * np.cos((2*self.k1 - self.k2 + self.k3)*t)
        term3 = -0.5 * self.eta * (self.k2 + self.k3) * (1 + self.xi) * np.cos((2*self.k1 + self.k2 - self.k3)*t)
        return term1 + term2 + term3

    def omega_2(self, t):
        term1 = self.eta * (2*self.k1 + self.xi*self.k2 + self.xi*self.k3) * np.sin((self.k3 - self.k2)*t)
        term2 = 0.5 * self.eta * (self.k2 + self.k3) * (1 - self.xi) * np.sin((2*self.k1 - self.k2 + self.k3)*t)
        term3 = 0.5 * self.eta * (self.k2 + self.k3) * (1 + self.xi) * np.sin((2*self.k1 + self.k2 - self.k3)*t)
        return term1 + term2 + term3

    def omega_3(self, t):
        term1 = (2*self.xi*self.k1 + self.k2 + np.square(self.xi)*self.k2 - np.square(self.eta)*self.k3)
        term2 = (self.k2 - np.square(self.xi)*self.k2 + np.square(self.eta)*self.k3) * np.cos(2*self.k1*t)
        return term1 + term2

    def calculate_omega_values(self):
        time_steps = np.arange(self.start_time, self.end_time + self.delta_t, self.delta_t)
        omega1 = self.omega_1(time_steps)
        omega2 = self.omega_2(time_steps)
        omega3 = self.omega_3(time_steps)
        return time_steps, omega1, omega2, omega3

    def calculate_lambda_values(self):
        time_steps = np.arange(self.start_time, self.end_time + self.delta_t, self.delta_t)
        lambda0 = np.cos(self.k1 * time_steps) * np.cos(self.k2 * time_steps) - self.xi * np.sin(self.k1 * time_steps) * np.sin(self.k2 * time_steps)
        lambda1 = self.eta * np.sin(self.k1 * time_steps) * np.cos(self.k3 * time_steps)
        lambda2 = self.eta * np.sin(self.k1 * time_steps) * np.sin(self.k3 * time_steps)
        lambda3 = np.cos(self.k1 * time_steps) * np.sin(self.k2 * time_steps) + self.xi*np.sin(self.k1 * time_steps)*np.cos(self.k2 * time_steps)
    ...
    def plot_lambda_2d(self, ax_to_plot, right_frame):
        ...
        if ax_to_plot == "ax1":
            ...
            ax.set_xlabel('λ0')
            ax.set_ylabel('λ1')
            ax.set_title('Траєкторія λ₁ vs λ₀')
        elif ax_to_plot == "ax2":
    ...
    def plot_omega(self, right_frame, selected_checkboxes):
        ...
        fig_height = 3 * subplot_count if subplot_count > 0 else 4
...
if __name__ == "__main__":
    root = ThemedTk(theme="breeze")  # Виберіть тему, яка вам подобається
    app = MyApp(root)
